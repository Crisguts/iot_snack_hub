...
if (fridgeModal) {
  fridgeModal.addEventListener('show.bs.modal', (event) => {
    ...
    window.tempGaugeInstance = new Chart(ctxTemp, {
      type: 'doughnut',
      data: { datasets: [{ data: [data.temperature, 50 - data.temperature], backgroundColor: ['#ff4d4d', '#555'] }] },
      options: { responsive: true, cutout: '70%', plugins: { legend: { display: false } } }
    });

    // Humidity Gauge
    const ctxHumidity = document.getElementById('humidityGaugeModal').getContext('2d');
    window.humidityGaugeInstance = new Chart(ctxHumidity, {
      type: 'doughnut',
      data: { datasets: [{ data: [data.humidity, 100 - data.humidity], backgroundColor: ['#3399ff', '#555'] }] },
      options: { responsive: true, cutout: '70%', plugins: { legend: { display: false } } }
    });

    // Historical Line Chart
    const ctxHistory = document.getElementById('iotChartModal').getContext('2d');
...
document.querySelectorAll('.fanToggle').forEach(toggle => {
  toggle.addEventListener('change', function() {
    ...
    fetch(`/fan/${fridgeId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' } })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
...
if (testEmailBtn) {
  testEmailBtn.addEventListener('click', () => {
    ...
    fetch('/api/email/test', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
      .then(res => res.json())
      .then(data => {
        alert(data.success ? 'Test email sent! Reply "YES" to activate fan.' : 'Failed: ' + data.error);
      })
      .finally(() => {
  ...
  fetch('/api/email/check-signals')
    ...
    .then(data => {
      if (data.success && data.fan_activated) {
        ...
        const toggle = document.querySelector(`.fanToggle[data-fridge="${fridgeId}"]`);
        if (toggle) {
          toggle.checked = true;
          updateFanAnimation(fridgeId, true);
        }
        showNotification(`Fan activated for Fridge ${fridgeId} via email reply!`, 'success');
      }
    });
}
  ...
  try {
    ...
    if (result.success && result.data) {
      for (const fridgeId in result.data) {
        ...
        if (isModalOpen) {
          const currentFridge = document.querySelector('#fridgeModal')?.getAttribute('data-current-fridge');
          if (currentFridge == fridgeId) {
